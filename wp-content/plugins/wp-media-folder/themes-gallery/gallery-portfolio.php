...
foreach ($attachments as $id => $attachment) {
    ...
    if ($customlink) {
        ...
        $icon .= '<a class="portfolio_lightbox ' . $tclass . '" href="' . $url_image . '" title="' . $post_title . '" target="' . $link_target . '">+</a>';
        if ($url_image === '') {
            $url_image = get_attachment_link($id);
        }
  ...
  title="' . $post_title . '" target="' . $link_target . '">+</a>';
            ...
            if ($targetsize) {
                if (get_post_meta($id, _WPMF_GALLERY_PREFIX . 'custom_image_link', true) !== '') {
                    ...
                    $lb        = 1;
                    $img       = wp_get_attachment_image_src($id, $targetsize);
                    $url_image = $img[0];
                }
            } else {
                $url_image = $urls[0];
 ...
 title="' . $post_title . '" class="hover_img ' . $tclass . ' isvideo" target="' . $link_target . '"></a>
 <a data-lightbox="1" class="portfolio_lightbox ' . $tclass . ' isvideo" href="' . $remote_video . '"
  title="' . $post_title . '">+</a>';
            } else {
                $icon = '<a data-lightbox="' . $lb . '" href="' . $url_image . '"
 title="' . $post_title . '" class="hover_img ' . $tclass . ' not_video"></a>
 <a data-lightbox="1" class="portfolio_lightbox ' . $tclass . ' not_video" href="' . $url_image . '"
  title="' . $post_title . '">+</a>';
            }
        }
 ...
 <a class="portfolio_lightbox ' . $tclass . '" href="' . $url_image . '" title="' . $post_title . '" target="' . $link_target . '">+</a>';
            ...
            $image_output = wp_get_attachment_image($id, $size . ' skip-lazy', false, array('data-type' => 'wpmfgalleryimg'));
