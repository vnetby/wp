...
l=g.width,m=g.height;if(0>=l||0>=m)a.empty().append(document.createTextNode(wpmf_pdfemb_trans.objectL10n.widthheightinvalid));else{var q=a.parent().width(),g=l,p=m;if("max"==a.data("width"))g=q;else if("auto"==a.data("width"))g=l;else if(g=parseInt(a.data("width"),10),isNaN(g)||0>=g)g=q;0>=g&&(g=l);g>q&&0<q&&(g=q);n=g/l;var p=m*n,w=a.find("div.pdfemb-toolbar-fixed"),t=(q=pdfembWantMobile(c,a,g,k))?0:w.length,k=parseInt(a.data("height"),10);if(isNaN(k)||0>=k||k>p)k="auto"==a.data("height")?a.parent().height()-
...
a.data("pagenum"),!0);a.data("fromZoom",b).data("toZoom",e)};c.fn.pdfEmbedder.addToolbar=function(a,d,b,e){var f=c("<div></div>",{"class":"pdfemb-toolbar pdfemb-toolbar"+(b?"-fixed":"-hover")+" "+(d?" pdfemb-toolbar-top":"pdfemb-toolbar-bottom")}),h=c('<button class="pdfemb-prev" title="'+wpmf_pdfemb_trans.objectL10n.prev+'" type="button"><span class="sr-only">'+wpmf_pdfemb_trans.objectL10n.prev+'</span></button>');f.append(h);var r=c('<button class="pdfemb-next" title="'+wpmf_pdfemb_trans.objectL10n.next+'" type="button"><span class="sr-only">'+wpmf_pdfemb_trans.objectL10n.next+'</span></button>');f.append(r);f.append(c('<div class="pdfemb-page-area">'+
wpmf_pdfemb_trans.objectL10n.page+' <span class="pdfemb-page-num">0</span> / <span class="pdfemb-page-count"></span></div>'));var u=c('<button class="pdfemb-zoomout" title="'+wpmf_pdfemb_trans.objectL10n.zoomout+'" type="button"><span class="sr-only">'+wpmf_pdfemb_trans.objectL10n.zoomout+'</span></button>');f.append(u);var v=c('<button class="pdfemb-zoomin" title="'+wpmf_pdfemb_trans.objectL10n.zoomin+'" type="button"><span class="sr-only">'+wpmf_pdfemb_trans.objectL10n.zoomin+'</span></button>');f.append(v);f.append(c("<div>"+wpmf_pdfemb_trans.objectL10n.zoom+' <span class="pdfemb-zoom">100%</span></div>'));e&&f.append(c("<div>"+wpmf_pdfemb_trans.objectL10n.secure+
...
a.data("zoom")||c.fn.pdfEmbedder.changeZoom(a,-10)});pdfembAddMoreToolbar(c,f,a);b||(a.on("mouseenter",function(b){b=a.find("div.pdfemb-toolbar-hover");!0!==b.data("no-hover")&&b.show()}),a.on("mouseleave",function(b){a.find("div.pdfemb-toolbar-hover").hide()}));wpmf_pdfemb_trans.poweredby&&f.append(c("<div></div>",{"class":"pdfemb-poweredby"}).append(c('<a href="https://wp-pdf.com/?utm_source=Poweredby&utm_medium=freemium&utm_campaign=Freemium" target="_blank">wp-pdf.com</a>')))};PDFJS.workerSrc=wpmf_pdfemb_trans.worker_src;
PDFJS.cMapUrl=wpmf_pdfemb_trans.cmap_url;PDFJS.cMapPacked=!0;c(".wpmf-pdfemb-viewer[data-wpmf_pdf_embed='embed']").pdfEmbedder()});
